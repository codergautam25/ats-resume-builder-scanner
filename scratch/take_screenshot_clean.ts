...
const userRawText = `429.6679545971678Bangalore 492.374979484375+91-7980876391
...
async function main() {
  ...
  // Paste user raw text into text area
  await page.fill('textarea[placeholder*="Paste your existing resume text"]', userRawText);
  await page.waitForTimeout(1000);

  const screenshotPath = path.join(process.cwd(), 'scratch', 'clean_textarea_proof.png');
  await page.screenshot({ path: screenshotPath, fullPage: false });

  console.log('Clean textarea proof screenshot saved to:', screenshotPath);
  await browser.close();
}
