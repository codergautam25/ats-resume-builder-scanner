import { parseResumeTextToStructuredData, stripPdfCoordinateNoise } from '../src/utils/resumeParser';
...
const cleaned = stripPdfCoordinateNoise(rawIndraniText);
const parsed = parseResumeTextToStructuredData(cleaned);
...
sanitized.experience.forEach((j, i) => console.log(`Job ${i + 1}: ${j.position} at ${j.company} (${j.startDate} - ${j.endDate})`));
